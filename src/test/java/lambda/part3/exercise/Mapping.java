...
public class Mapping {
    ...
    private List<JobHistoryEntry> addOneYear(List<JobHistoryEntry> jobHistory) {
        final List<JobHistoryEntry> list = new ArrayList<>();
        jobHistory.forEach(entry -> list.add(entry.withDuration(entry.getDuration() + 1)));
        return list;
    }

    private List<JobHistoryEntry> repalceQA(List<JobHistoryEntry> jobHistory) {
        final List<JobHistoryEntry> list = new ArrayList<>();
        jobHistory.forEach(entry -> {
            final JobHistoryEntry modifiedEntry = entry.getPosition().equals("qa") ? entry.withPosition("QA") : entry;
            list.add(modifiedEntry);
        });
        return list;
    }
    ...
    public void mapping() {
        ...
        final List<Employee> mappedEmployees =
                new MapHelper<>(employees)
                        ...
                        .map(e -> e.withJobHistory(repalceQA(e.getJobHistory())))
                        .getList();
    ...
    private static class LazyFlatMapHelper<T, R> {
        ...
        private <R2> Function<R, List<R2>> rR2TorListR2(Function<R, R2> f) {
            final List<R2> r2List = new ArrayList<>();
            return r -> {
                r2List.add(f.apply(r));
                return r2List;
            };
        }
