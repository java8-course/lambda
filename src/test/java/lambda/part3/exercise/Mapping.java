...
public class Mapping {
    ...
    private List<JobHistoryEntry> changePosition(List<JobHistoryEntry> jobHistory) {
        return new MapHelper<>(jobHistory)
                .map(jobEntry ->
                        jobEntry.getPosition().equals("qa") ? jobEntry.withPosition("QA") : jobEntry)
                .getList();
    }
    ...
    private static class LazyMapHelper<T, R> {
        ...
        public <R2> LazyMapHelper<T, R2> map(Function<R, R2> f) {
            ...
            return new LazyMapHelper<>(this.list, combinedFunction);
        }

        private <R2> Function<T, R2> combine(Function<T, R> function, Function<R, R2> f) {
            if (f == null)
                return (Function<T, R2>) function;
    ...
    private static class LazyFlatMapHelper<T, R> {
        ...
        public LazyFlatMapHelper<T, T> filter(Predicate<T> p) {
            ...
            return LazyFlatMapHelper.from(result);
        }


        public <R> LazyFlatMapHelper<T, R> map(Function<T, R> f) {
            final Function<T, List<R>> listFunction = rR2TorListR2(f);
            return flatMap(listFunction);
        }

        // (R -> R2) -> (R -> [R2])
        private <R> Function<T, List<R>> rR2TorListR2(Function<T, R> f) {
            return r -> Collections.singletonList(f.apply(r));
        }

        // TODO *
        public <R> LazyFlatMapHelper<T, R> flatMap(Function<T, List<R>> f) {
