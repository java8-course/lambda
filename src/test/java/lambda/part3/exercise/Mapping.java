...
public class Mapping {
    ...
    private static class MapHelper<T> {
        ...
        public <R> MapHelper<R> map(Function<T, R> f) {
            ...
            list.forEach(t -> r.add(f.apply(t)));
            return new MapHelper<R>(r);
        }
    ...
    public void mapping() {
        ...
        final List<Employee> mappedEmployees =
                new MapHelper<>(employees)
                        ...
                        .map(e -> e.withJobHistory(changePosition(e.getJobHistory()))).getList();
    ...
    private List<JobHistoryEntry> changePosition(List<JobHistoryEntry> jobHistory) {
        return new MapHelper<>(jobHistory).map(j -> j.getPosition().equals("qa") ? j.withPosition("QA") : j).getList();
    }

    private List<JobHistoryEntry> addOneYear(List<JobHistoryEntry> jobHistory) {
        return new MapHelper<>(jobHistory).map(h -> h.withDuration(h.getDuration() + 1)).getList();
    }
    ...
    private static class LazyMapHelper<T, R> {
        ...
        public List<R> force() {
            List<R> r = new ArrayList<>();
            list.forEach(t -> r.add(function.apply(t)));
            return r;
        }
    ...
    public void lazy_mapping() {
        ...
        final List<Employee> mappedEmployees =
                LazyMapHelper.from(employees)
                        .map(e -> e.withPerson(e.getPerson().withFirstName("John")))
                        .map(e -> e.withJobHistory(addOneYear(e.getJobHistory())))
                        .map(e -> e.withJobHistory(changePosition(e.getJobHistory()))).force();
