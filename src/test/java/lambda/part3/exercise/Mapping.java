...
public class Mapping {
    ...
    public void mapping() {
        ...
        final List<Employee> mappedEmployees =
                ...
                .map(TODO) // replace qa with QA */
                        ...
                        .map(e -> e.withJobHistory(mapToQA(e.getJobHistory())))
                        .getList();
    ...
    private static class LazyFlatMapHelper<T, R> {
        ...
        private <R2> Function<R, List<R2>> rR2TorListR2(Function<R, R2> f) {
            return r -> Arrays.asList(f.apply(r));
        }

        // TODO *
        public <R2> LazyFlatMapHelper<T, R2> flatMap(Function<R, List<R2>> f) {
            Function<T, List<R2>> func = t -> {
                final List<R2> result = new ArrayList<R2>();
                function.apply(t).forEach(r -> f.apply(r).forEach(result::add));
                return result;
            };
            return new LazyFlatMapHelper<T, R2>(list, func);
        }
    }
    ...
    public void lazy_filtering() {
        ...
        final List<Employee> expectedResult1 =
                Arrays.asList(
                        new Employee(
                                new Person("John", "Galt", 30),
                                Arrays.asList(
        ...
        assertEquals(filteredEmployees, expectedResult1);
    }
}
