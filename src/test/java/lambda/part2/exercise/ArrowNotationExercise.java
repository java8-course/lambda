...
public class ArrowNotationExercise {
    ...
    private static String getFullName(Person p){return p.getFirstName()+' '+p.getLastName();}

    // TODO

    private static BiFunction<Person,Person,Integer> ageOfPersonWithTheLongestFullName(Function<Person,String > func) {
        return (person, person2) -> {
            String pFullName = func.apply(person);
            String p2FullName = func.apply(person2);

            if (pFullName.length() > p2FullName.length()){
